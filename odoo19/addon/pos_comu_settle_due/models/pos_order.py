...
class PosOrder(models.Model):
    ...
    customer_due_total = fields.Monetary(
        string="Due", compute="_compute_customer_due_total", store=True, currency_field="currency_id")
    init_customer_due_total = fields.Monetary(
        string="Initial due (before any settlement)", currency_field="currency_id")
    settled_order_line_ids = fields.One2many(
        "pos.order.line", "settled_order_id", string="Settled Order Lines")
    settled_orders_count = fields.Integer(
        string="Number of settled orders", compute='_compute_settled_orders_count', store=True)
    commercial_partner_id = fields.Many2one(
        comodel_name="res.partner", related="partner_id.commercial_partner_id", readonly=True, store=True)
        ...
        for order in self:
            ...
            if order.partner_id and order_pay_later_pm and not order.is_invoiced:
                if order.customer_due_total:
                    ...
                    order_due = sum(order.payment_ids.filtered(lambda payment: payment.amount >
                                    0 and payment.payment_method_id.type == 'pay_later').mapped('amount'))
                    customer_due = order.partner_id.get_total_due(
                        order.config_id.id)['res.partner'][0]['total_due']
