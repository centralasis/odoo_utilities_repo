...
class PosSession(models.Model):
    ...
    def _reconcile_account_move_lines(self, data):
        ...
        pay_later_move_lines |= self.order_ids.mapped('lines').filtered(
            lambda l: (
                l.settled_invoice_id or l.settled_order_id) and l.order_id.account_move
        ...
        if pay_later_move_lines:
            ...
            for (partner_id, account_id), move_lines in partner_account_lines.items():
                ...
                (self.env['account.move.line'].browse([l.id for l in move_lines])
                 | session_move_lines | invoice_move_lines).reconcile()
        return data
