...
from escpos.printer import Network as POSLanDriver
...
SUPPORTED_DEVICES = [
    ...
    {"vendor": 0x1FC9, "product": 0x2016, "name": "EPOS"},
...
try:  # noqa C901
    ...
    class ESCPOSDriver(ThreadDriver, POSDriver):
        ...
        def receipt_jpeg(self, b64image):
            ...
            TARGET_WIDTH = 576
            img = Image.open(io.BytesIO(base64.b64decode(b64image)))
            w_percent = (TARGET_WIDTH / float(img.size[0]))
            h_size = int((float(img.size[1]) * float(w_percent)))

            # LANCZOS provides clean downscaling for sharp text/lines
    ...
    def default_printer_action():
        ...
        if action == "print_receipt":
            ...
            TARGET_WIDTH = 576
            img = Image.open(io.BytesIO(base64.b64decode(receipt)))
            w_percent = (TARGET_WIDTH / float(img.size[0]))
            h_size = int((float(img.size[1]) * float(w_percent)))
            ...
            printer = POSLanDriver("192.168.1.109", port=9100)
            printer.image(resized_img)
            printer.cut()
            printer.close()
